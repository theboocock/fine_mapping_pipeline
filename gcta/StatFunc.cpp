...
double StatFunc::chi_val(double df, double prob)
{
	...
	if(prob_buf > prob ) { preway=way=1.0; chi_val+=walk; }
	else { preway=way=-1.0; chi_val-=walk;  }
...
double StatFunc::t_val(double df, double prob)
{
	...
	if(prob_buf > prob ) { preway=way=1.0; t_val+=walk; }
	else { preway=way=-1.0; t_val-=walk;  }
...
double StatFunc::F_val(double df_1, double df_2, double prob)
{
	...
	if(prob_buf > prob ) { preway=way=1.0; F_val+=walk; }
	else { preway=way=-1.0; F_val-=walk;  }
...
{
    if(x>=0) return(0.5*(1+erf(x/pow(2,0.5))));
    else return(0.5*(1-erf(fabs(x)/pow(2,0.5))));
}

double StatFunc::dnorm(double x){ return(0.39894228*exp(-0.5*pow(x,2))); }

double StatFunc::qnorm_sub(double x,double y){ return(y+0.5*x*pow(y,2)+(2*pow(x,2)+1)*pow(y,3)/6+(6*pow(x,3)+7*x)*pow(y,4)/12);}

double StatFunc::qnorm(double p, bool upper)
{
    double x=0;
    if(upper) p=1-p;
    for(int i=0; i<4; i++) x=x+qnorm_sub(x,(1-p-pnorm(x))/dnorm(x));
    return(x);
}
