...
void outputStatistics(double totalScore, double nullModelPrior, \
	...
	double likelihoodUpToAConstant = totalScore;
	double ProbabilityAtLeast1Causal =  1 - nullModelPrior / totalScore;
	double globalBayesFactor = ( totalScore - nullModelPrior ) /  \
	                           (1 - nullModelPrior);
	outputStream << std::scientific << std::setprecision(15) \
	         << likelihoodUpToAConstant << " # p(Data) / p(Data|M0)" << endl;
	cout << likelihoodUpToAConstant << \
	             " # p(Data) / p(Data|M0)" << endl;
	...
	outputStream << globalBayesFactor << \
	       " # Bayes factor of the region (M_alternative vs M0)" << endl;
...
void greedySearch(BayesFactorData& bfData, double* modelScore, \
	...
	for (unsigned int step = 1; step <= nSNP; step++) {
